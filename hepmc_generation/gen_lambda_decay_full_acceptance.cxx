...
void gen_lambda_decay_full_acceptance(int n_events = 10000, 
                    const char* out_fname = "gen_lambda_decay_full_acceptance.hepmc", 
                  ...
                  )
{
  ...
  while (accepted_events < n_events) {
    ...
    else if(dist == "discrete")
    {
      ...
      const int num_loguniform_energies = 35; // 36 for up to 300 GeV, 49 for up to 1 TeV, 34 for 250 GeV
      const int random_power = (int) r1->Uniform(23, num_loguniform_energies); // 93-274 GeV
